...
//
// LC-ARCH-004 Phase 2: this test now exercises `runtimeCore.js` directly — the module that used to
// live at `backend/src/modules/runtime/runtime.service.js` before that became an HTTP client to
// this standalone process (see `../src/server.js`). Stub paths below resolve to the SAME absolute
// backend files `runtimeCore.js` itself requires (it computes them the same way, relative to its
// own file), so intercepting via `require.cache` here reaches the real call sites.
//
// LC-ARCH-003 Phase 1.1 boundary (unchanged by the process split): Runtime resolves ONLY
// `liveControllerBinding`/`livePointBinding` (the deployed projection) — never
// `controllersMapped`/`pointsMapped` (Engineering's mutable working assignment) directly. This
// test's Engineering-side setup (`controllers`/`mappings`, populated via the real
// `syncSimCatalogBindingsForEquipmentId` helper) is deliberately kept separate from the Live tables
// Runtime actually reads; `materializeLive()` stands in for what `siteVersion.service.js`'s deploy
// transaction would do, without pulling in the whole SiteVersion machinery into a test that is
// really about poll-loop mechanics.
...
stubAbsolute(path.join(backendSrc, 'modules', 'alarms', 'alarm.service'), { evaluateForPointIds: async () => { alarmEvaluations += 1; } });

const runtime = require('../src/core/runtimeCore');
const { store } = require('../src/core/runtime.store');
const { syncSimCatalogBindingsForEquipmentId } = require(path.join(backendSrc, 'lib', 'simCatalogBindingSync'));
const pointService = require(path.join(backendSrc, 'modules', 'points', 'point.service'));
...
async function main() {
  ...
  // LC-ARCH-004: present value/quality land in PointRuntimeState (Runtime's own table), not just
  // the legacy Point mirror.
  const prsForTemp = pointRuntimeStates.find((r) => r.pointId === temp.id);
  assert(prsForTemp, 'a PointRuntimeState row exists for the polled point');
  assert.equal(prsForTemp.presentValue, temp.presentValue, 'PointRuntimeState mirrors the same value written to Point');
  assert.equal(prsForTemp.quality, 'GOOD', 'a freshly-polled point is quality GOOD');
  assert.equal(prsForTemp.source, 'SIM', 'runtime-state records which protocol driver produced the value');
  const crsForCtrl = controllerRuntimeStates.find((r) => r.liveControllerBindingId === liveControllerBindings.find((l) => l.equipmentId === 'eq-0').id);
  assert.equal(crsForCtrl.status, 'ONLINE');
  assert.equal(crsForCtrl.quality, 'GOOD');
  ...
  console.log('PASS: SIM assignment isolation, Engineering/Live binding boundary, PointRuntimeState/ControllerRuntimeState quality, changing values, alarm evaluation, binding preservation, freshness, stop/restart, write failure, and overlapping polls');
}
