...
export default function EquipmentTemplatesTable({
  templates,
  onView,
  onEdit,
  onDuplicate,
  ...
  if (!templates || templates.length === 0) {
    return (
      <div className="p-4 text-center text-white-50 small">
        No equipment templates in this site. Import from Global Library or create one.
      </div>
    );
  }
        ...
        <tbody>
          {templates.map((row) => (
            <tr key={row.id} className="discovery-table-row">
              ...
              <td className="text-white-50">{row.equipmentType}</td>
              <td className="text-white-50">{(row.points && row.points.length) ?? row.pointCount ?? 0}</td>
              <td className="text-white-50">{row.defaultGraphic || "—"}</td>
              <td>
                <span
