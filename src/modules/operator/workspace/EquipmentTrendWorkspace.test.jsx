...
beforeEach(() => {
  localStorage.clear();
  vi.spyOn(operatorDefinitionsRepository, "fetchTrendStore").mockResolvedValue({ definitions: [], assignments: [] });
  vi.spyOn(operatorDefinitionsRepository, "saveDefinition").mockResolvedValue({ id: "saved" });
  vi.spyOn(operatorRepository, "getTrendPointCatalog").mockReturnValue([
...
it("validates required points before allowing template assignment and reports success/failure per target", async () => {
  ...
  vi.spyOn(operatorDefinitionsRepository, "assignDefinition").mockResolvedValue({});
  vi.spyOn(operatorRepository, "getWorkspacePointsForEquipment").mockImplementation((id) => {
