...
it("waits for durable schedule save before updating occupancy rows", async () => {
  let complete;
  const save = jest.spyOn(operatorDefinitionsRepository, "saveDefinition").mockImplementation(() => new Promise((resolve) => { complete = resolve; }));
  ...
  jest.spyOn(operatorDefinitionsRepository, "fetchSchedules").mockResolvedValue(rows);
  const changed = jest.fn();
  const view = render(<EquipmentOccupancyWorkspace equipment={{ id: "fcu-1", name: "FCU-1" }} schedules={[]} occupancy={{ label: "Unoccupied" }} currentUser={{ roleKey: "operator" }} now={new Date()} siteKey="saved-site" onBack={jest.fn()} onSchedulesChange={changed} />);
